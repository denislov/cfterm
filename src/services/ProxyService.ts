...
export class ProxyService {
	...
	async handleUpgrade() {
		...
		this._processStream(server).catch((err) => {
	...
	async connectStreams(remoteSocket: Socket, webSocket: WebSocket, headerData: Uint8Array<ArrayBuffer> | null, retryFunc: (() => Promise<void>) | null) {
		...
		await remoteSocket.readable.pipeTo(
			new WritableStream({
				async write(chunk, controller) {
					...
					if (header) {
						// 使用 TypedArray 合并数据，比 Blob 更高效
						const chunkArray = new Uint8Array(chunk);
						const combined = new Uint8Array(header.length + chunkArray.length);
						combined.set(header);
						combined.set(chunkArray, header.length);
		...
		).catch((_error) => { Utils.closeSocketQuietly(webSocket); });
		if (!hasData && retryFunc) await retryFunc();
	}

	async _processStream(serverSocket: WebSocket) {
		// WebSocket 流处理：解析 VLESS 协议并转发流量
		const earlyData = this.ctx.request.headers.get(atob('c2VjLXdlYnNvY2tldC1wcm90b2NvbA==')) || '';
		const readableStream = this._makeReadableStream(serverSocket, earlyData);

		let remoteConnWrapper: { socket: Socket | null; writer: WritableStreamDefaultWriter | null } = { socket: null, writer: null };
		...
		await readableStream.pipeTo(new WritableStream({
			write: async (chunk) => {
				// DNS 查询继续转发
				if (isDnsQuery) {
					return await this.forwardUDP(chunk, serverSocket, null);
				}

				// 已建立连接，复用 writer 转发数据
				...
				if (!protocolType) {
					if (this.ctx.kvConfig.ev && chunk.byteLength >= 24) {
						...
						if (!vlessResult.hasError) {
							...
							if (isDnsQuery) return this.forwardUDP(rawData, serverSocket, respHeader);
							await this.forwardTCP(addressType!, hostname!, port!, rawData, serverSocket, respHeader, remoteConnWrapper);
							return;
						}
					}
	...
	async forwardTCP(addrType: number, host: string, portNum: number, rawData: any, ws: WebSocket, respHeader: Uint8Array<ArrayBuffer>, remoteConnWrapper: { socket: Socket | null; }) {
		...
		const enableSocksDowngrade = this.ctx.kvConfig?.enableSocksDowngrade;
		const isSocksEnabled = this.ctx.kvConfig?.isSocksEnabled;
	...
	_parseWsPacketHeader(chunk: any, token: string) {
		...
		switch (addressType) {
			...
			case ADDRESS_TYPE.IPV6: addrLen = 16; const ipv6 = []; const ipv6View = new DataView(chunk.slice(addrValIdx, addrValIdx + addrLen)); for (let i = 0; i < 8; i++) ipv6.push(ipv6View.getUint16(i * 2).toString(16)); hostname = ipv6.join(':'); break;
