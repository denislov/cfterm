...
export class Utils {
	...
	static async sha224(text: string) {
		...
		for (let chunk = 0; chunk < paddedLen; chunk += 64) {
			...
			for (let i = 16; i < 64; i++) {
				const s0 = Utils.rightRotate(W[i - 15], 7) ^ Utils.rightRotate(W[i - 15], 18) ^ (W[i - 15] >>> 3);
				const s1 = Utils.rightRotate(W[i - 2], 17) ^ Utils.rightRotate(W[i - 2], 19) ^ (W[i - 2] >>> 10);
			...
			for (let i = 0; i < 64; i++) {
				const S1 = Utils.rightRotate(e, 6) ^ Utils.rightRotate(e, 11) ^ Utils.rightRotate(e, 25);
				const ch = (e & f) ^ (~e & g);
				const temp1 = (h + S1 + ch + K[i] + W[i]) >>> 0;
				const S0 = Utils.rightRotate(a, 2) ^ Utils.rightRotate(a, 13) ^ Utils.rightRotate(a, 22);
	...
	static rightRotate(value: number, amount: number) {
